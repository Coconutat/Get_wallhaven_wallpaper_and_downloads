...
def multipleimgpage():  # 多图片页面解析
    ...
    with open("imglist.json", "a", encoding="UTF-8") as f:  # 自己定义输出json文件的位置
        f.write(json.dumps(real_imgstore, ensure_ascii=False, indent=2))

# https://wallhaven.cc/w/l3zyg2
def singleimgpage(): # 单图片页面解析
    ...
    singledemo = rs.text
    singlesoup = BeautifulSoup(singledemo, "html5lib")  # html5lib / html.parser
